        ...
        UnityEngine_Debug =
            {
                ...
                LogWarning: (...args) => CSConsole.WriteLine(["[LogWarn]", ...args].join(' ')),
...
if (UnityEngine_Debug || !global.console) {
    ...
    console.trace = function () {
        if (console_org)
            console_org.trace.apply(null, Array.prototype.slice.call(arguments));
        UnityEngine_Debug.Log(toString(arguments) + "\n" + getStack(new Error()) + "\n");
    };
    console.assert = function (condition) {
        if (console_org)
            console_org.assert.apply(null, Array.prototype.slice.call(arguments));
        if (condition)
            return;
