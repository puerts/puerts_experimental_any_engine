...
function referWrapperMember(wrapperName, contstructorName, methodName, propertyGetterName, propertySetterName) {
    const ret = [];
    if (contstructorName)
        ret.push(`Constructor = ${wrapperName}.${contstructorName}`);
    if (methodName)
        ret.push(`Method = ${wrapperName}.${methodName}`);
    if (propertyGetterName)
        ret.push(`PropertyGetter = ${wrapperName}.${propertyGetterName}`);
    if (propertySetterName)
        ret.push(`PropertySetter = ${wrapperName}.${propertySetterName}`);
