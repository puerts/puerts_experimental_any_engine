...
export default function TypingTemplate(data, csharpModuleWillGen) {
    ...
    function t(str, ...values) {
        ...
        for (var i = 1; i < lines.length; i++) {
            if (lines[i])
                newLines.push(append + lines[i].replace(/^[\t\s]*/, ''));
        }
        ret += newLines.join('\n');
    }
    const baseIndent = 0;
        ...
        toJsArray(ns.Types).forEach(type => {
            ...
            if (!(type.Name == 'JSObject' && ns.Name == 'Puerts')) {
                ...
                if (type.IsDelegate) {
                    ...
                    if (((_a = type.IteratorReturnName) === null || _a === void 0 ? void 0 : _a.length) > 0) {
                        !type.IsInterface && t `public `;
                ...
                if (type.ExtensionMethods.Length > 0 && !type.IsEnum) {
                    ...
                    toJsArray(type.ExtensionMethods).forEach(method => {
                        ...
                        t `
                        ${method.Document}
                        ${formatPropertyOrMethodName(method.Name)} (${toJsArray(method.ParameterInfos).map((pinfo, idx) => parameterDef(pinfo)).join(', ')}) : ${method.TypeName};
                        `;
                    });
                    t.indent = 8 + baseIndent;
                    t `
