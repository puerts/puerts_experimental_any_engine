...
export default function TaggedTemplateEngine(str, ...exps) {
    ...
    while (i < str.length - 2) {
        ...
        // handle exp
        if (exps[i] == IF)
            throw new Error('tte.IF must be used as a function');
        if (exps[i] == ENDIF)
            throw new Error('tte.ENDIF must be used as a function');
        if (exps[i] == ELSE)
            throw new Error('tte.ELSE must be used as a function');
        if (exps[i] == ELSEIF)
            throw new Error('tte.ELSEIF must be used as a function');
        if (ifStack.length) {
            if (exps[i] === TTEndif) {
                ifStack.pop();
    ...
    function WillConnectWithAnEmptyLine(str1, str2) {
        ...
        if (str1LastLineSeq != -1 && str1.substring(str1LastLineSeq).trim() == "" &&
...
export function FOR(arr, fn, joiner = '') {
    ...
    return new TTFor(ret
        ...
        if (str.substring(lastLineSeq).trim() == "")
            return str.substring(0, lastLineSeq);
        else
            return str;
    })
        .join(joiner));
