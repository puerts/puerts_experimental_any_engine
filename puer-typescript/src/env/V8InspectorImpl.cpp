...
#if !defined(WITHOUT_INSPECTOR) && !defined(WITH_QUICKJS)
...
{
// std::vector<V8InspectorPlugin*> V8Inspector::plugins = std::vector<V8InspectorPlugin*>();
// bool V8Inspector::RegisterPlugin(V8InspectorPlugin* plugin) {
...
class V8InspectorClientImpl : public V8InspectorEx,
#if USING_UE
...
public:
    ...
    bool IsAnyInspectorConnected() override
    {
        return Connected;
    }
    void SendMessagePayload(wspp_connection_hdl handle, const std::string& payload) override
	{
		OnSendMessage(handle, payload);
	}
...
{
	websocketpp::lib::error_code ec;
	auto Connection = Server.get_con_from_hdl(Handle, ec);
    auto Resource = Connection->get_resource();
    auto incomeOrigin = Connection->get_request_header("Host");
    auto indexOfQ = Resource.find("?");
	auto indexOfS = Resource.find("#");
	if (ec)
		goto fail;
    if (indexOfQ > 0) Resource = Resource.substr(0, indexOfQ);
    if (indexOfS > 0) Resource = Resource.substr(0, indexOfS);
...
#endif
        auto json = JSONList;
		Connection->set_body(json.replace(json.find("127.0.0.1:9222"), 14, incomeOrigin), ec);
		if (ec)
			goto fail;
		Connection->set_status(websocketpp::http::status_code::ok, ec);
		if (ec)
			goto fail;
    }
...
#endif
		Connection->set_body(JSONVersion, ec);
		if (ec)
			goto fail;
		Connection->set_status(websocketpp::http::status_code::ok, ec);
		if (ec)
...
#endif
		Connection->set_body("404 Not Found", ec);
		if (ec)
			goto fail;
		Connection->set_status(websocketpp::http::status_code::not_found, ec);
		if (ec)
			goto fail;
    }
	return;
...
void V8InspectorClientImpl::OnOpen(wspp_connection_hdl Handle)
{
    ...
    auto HandlePtr = Handle.lock().get();
    V8InspectorChannels[HandlePtr] = channel;
    ...
    {
        for (auto iter = openListeners.begin() ; iter!= openListeners.end(); iter++)
        {
            auto func = *iter;
            func(Handle);
        }
...
void V8InspectorClientImpl::OnReceiveMessage(wspp_connection_hdl Handle, wspp_message_ptr Message)
{
    ...
    auto channel = V8InspectorChannels[HandlePtr];
    ...
    bool letV8Handle = true;
    if (messageListeners.size() > 0)
    {
        for (auto iter = messageListeners.begin() ; iter!= messageListeners.end(); iter++)
        {
            auto func = *iter;
            letV8Handle = letV8Handle && !func(Handle, payload);
        }
    }
    ...
    if (letV8Handle)
    {
        ...
        channel->DispatchProtocolMessage(payload);
    }
}
...
void V8InspectorClientImpl::OnSendMessage(wspp_connection_hdl Handle, const std::string& Message)
{
	...
	websocketpp::lib::error_code ec;
	Server.send(Handle, Message, websocketpp::frame::opcode::TEXT, ec);
	if (ec)
	{
#if USING_UE
...
void V8InspectorClientImpl::OnClose(wspp_connection_hdl Handle)
{
    ...
    {
        for (auto iter = closeListeners.begin() ; iter != closeListeners.end(); iter++)
        {
            auto func = *iter;
            func(Handle);
        }
